...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get user's internal ID - use correct column based on auth type
    ...
    // If user not found, create them using the unified helper
    if (userError && userError.code === 'PGRST116') {
      console.log(`[Global Balance API] User not found, creating new user for ${auth.type}: ${auth.userId}`);
      
      const { getOrCreateUserFromAuth } = await import('@/lib/user-management');
      user = await getOrCreateUserFromAuth(auth);
      } else if (userError) {
